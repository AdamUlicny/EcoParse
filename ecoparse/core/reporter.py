...
def generate_report(report_context: Dict[str, Any]) -> Optional[str]:
    ...
    # --- START OF FIX: All data is now sourced from the report_context dictionary ---
    ...
    report_data = {
        ...
        "gnfinder_info": {
            ...
            "taxonomic_filter_applied": bool(len(report_context.get('species_df_initial', [])) != len(report_context.get('species_df_final', []))),
            "total_species_identified_final_filter": len(report_context.get('species_df_final', []))
        },
        "llm_extraction_info": {
