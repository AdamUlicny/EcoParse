...
from ecoparse.core.sourcetext import get_species_context_chunks # Import for preview
import io
...
def display():
    ...
    if st.button("Start Extraction", type="primary", disabled=st.session_state.species_df_final.empty):
        ...
        source_context = {
            ...
            "examples_text": examples_text # Pass examples to the extractor
        }
        
        # Reset previous results and verification queue
        ...
        with st.spinner("LLM is processing..."):
            results = extractor.run_extraction(species_to_process, source_context, update_progress)
        
        st.session_state.extraction_results = results
        progress_bar.empty()
        st.success("Extraction complete! Proceed to the 'View Results' or 'Manual Verification' tab.")
        st.rerun()